...
def categorize_references_ids(references):
    ...
    rtypes = [
        item.split("?")[0] if "?" in item else item.split("/")[0] for item in references
    ]
    reference_ids = [
        item.split("|")[1] if "?" in item else item.split("/")[1] for item in references
    ]
...
def get_references_ndjson(reference_dict, key):
    ...
    with open(
        ...
        reference_idxs = {
            resource_ids[item] for item in reference_dict[key] if item in resource_ids
        }
